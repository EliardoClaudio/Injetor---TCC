...
public class VMController2 {
	...
	System.out.println("entrou no metodo isAlive da VM2 e retornou: " + aux);
     ...
     * @throws IOException 
     */
    public void runVMStateMachine2() throws InterruptedException, IOException {
	switch (this.getState()) {
	case RUNNING:
	    if (this.isAlive()) {
	    	int waitingTime = this.generateRandomFailureTime();
			this.setTimer(new MyTimer(waitingTime));
	    	WriteFile.logger("Started. VM2: " + "192.168.0.171", "Node_log.txt");
	...
	case TIMER_INJECT_FAILURE:
	    if (this.getTimer().isExpired()) {
			...
			int waitingTime = this.generateRandomFailureTime();
			this.setTimer(new MyTimer(waitingTime));
			WriteFile.logger("Inject Fault. VM2: " + "192.168.0.171", "Node_log.txt");
			WriteFile.logger(new Date().toString(), "Node_log.txt");
   			Thread.sleep(30000);
	...
	case FAILED:
	    if (!this.isAlive()) {
			...
			WriteFile.logger("Failed. VM2: " + "192.168.0.171", "Node_log.txt");
	    	WriteFile.logger(new Date().toString(), "Node_log.txt");
		    } else {
	...
	case TIMER_REPAIR:
	    if (this.getTimer().isExpired()) {
			...
			WriteFile.logger("Repaired. VM2: " + "192.168.0.171", "Node_log.txt");
			WriteFile.logger(new Date().toString(), "Node_log.txt");
   			Thread.sleep(30000);
			this.setState(StateMachineEnum.RUNNING);
			
			TestCCController2.Chamada2("192.168.0.153", "clouds");
			System.exit(0);
		    } else {
