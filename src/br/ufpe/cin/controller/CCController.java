...
/**
 * This class implements a CCController
 * 
...
public class CCController {
	...
	System.out.println("entrou no metodo isAlive do CC1 e retornou: " + aux);
     ...
     * @throws IOException 
     */
    public void runCCStateMachine() throws InterruptedException, IOException {
    	WriteFile.logger("Node: Conexão realizada com sucesso!", "Node_log.txt");
    	switch (this.getState()) {
    	case RUNNING:
    	    if (this.isAlive()) {
    	    	int waitingTime = this.generateRandomFailureTime();
    			this.setTimer(new MyTimer(waitingTime));
    	    	WriteFile.logger("Started. Node: " + this.getSshConnection().getHost(), "Node_log.txt");
    	    ...
    	    String statusUp = this.getState().toString();
    	...
    	case TIMER_INJECT_FAILURE:
    	    if (this.getTimer().isExpired()) {
    			...
    			int waitingTime = this.generateRandomFailureTime();
    			this.setTimer(new MyTimer(waitingTime));
    			WriteFile.logger("Inject Fault. Node: " + this.getSshConnection().getHost(), "Node_log.txt");
    			WriteFile.logger("Time Failure: " + waitingTime, "Node_log.txt");
    			WriteFile.logger(new Date().toString(), "Node_log.txt");
       			Thread.sleep(30000);
    	...
    	case FAILED:
    	    if (!this.isAlive()) {
    			...
    			WriteFile.logger("Failed. Node: " + this.getSshConnection().getHost(), "Node_log.txt");
    			WriteFile.logger("Time Failure: " + waitingTime, "Node_log.txt");
    	    	WriteFile.logger(new Date().toString(), "Node_log.txt");
    		    } else {
    	...
    	case TIMER_REPAIR:
    	    if (this.getTimer().isExpired()) {
    			...
    			WriteFile.logger("Time Failure: " + waitingTimeR, "Node_log.txt");
    			WriteFile.logger("Repaired. Node: " + this.getSshConnection().getHost(), "Node_log.txt");
    			WriteFile.logger(new Date().toString(), "Node_log.txt");
       			Thread.sleep(30000);
    			//this.setState(StateMachineEnum.RUNNING);
    			
       			TestVMStateMachine.Chamada("192.168.0.153", "clouds");
				System.exit(0);
