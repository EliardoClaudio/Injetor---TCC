...
public class CCController2 {
     ...
     * @throws IOException 
     */
    public void runCCStateMachine2() throws InterruptedException, IOException {
    	switch (this.getState()) {
    	case RUNNING:
    	    if (this.isAlive()) {
    	    	int waitingTime = this.generateRandomFailureTime();
    			this.setTimer(new MyTimer(waitingTime));
    	    	WriteFile.logger("Started. Node2: " + this.getSshConnection().getHost(), "Node_log.txt");
    	...
    	case TIMER_INJECT_FAILURE:
    	    if (this.getTimer().isExpired()) {
    			...
    			int waitingTime = this.generateRandomFailureTime();
    			this.setTimer(new MyTimer(waitingTime));
    			WriteFile.logger("Inject Fault. Node2: " + this.getSshConnection().getHost(), "Node_log.txt");
    			WriteFile.logger(new Date().toString(), "Node_log.txt");
       			Thread.sleep(30000);
    	...
    	case FAILED:
    	    if (!this.isAlive()) {
    			...
    			WriteFile.logger("Failed. Node2: " + this.getSshConnection().getHost(), "Node_log.txt");
    	    	WriteFile.logger(new Date().toString(), "Node_log.txt");
    		    } else {
    	...
    	case TIMER_REPAIR:
    	    if (this.getTimer().isExpired()) {
    			...
    			WriteFile.logger("Repaired. Node2: " + this.getSshConnection().getHost(), "Node_log.txt");
    			WriteFile.logger(new Date().toString(), "Node_log.txt");
       			Thread.sleep(30000);
    			//this.setState(StateMachineEnum.RUNNING);
       			TestVMStateMachine3.Chamada3("192.168.0.156", "clouds");
				System.exit(0);
